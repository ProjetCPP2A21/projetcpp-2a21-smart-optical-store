    ...
    : QObject(parent), ui(ui)
{
    ...
    connect(this->ui->comboShift_planning,
            &QComboBox::currentTextChanged,
            this,
            [this](const QString &shift) {

                if (shift == "MATIN") {
                    this->ui->timeEdit_debut_planning->setTime(QTime(8, 0));
                    this->ui->timeEdit_fin_planning->setTime(QTime(12, 0));
                }
                else if (shift == "APRES-MIDI") {
                    this->ui->timeEdit_debut_planning->setTime(QTime(13, 0));
                    this->ui->timeEdit_fin_planning->setTime(QTime(17, 0));
                }
                else if (shift == "SOIR") {
                    this->ui->timeEdit_debut_planning->setTime(QTime(18, 0));
                    this->ui->timeEdit_fin_planning->setTime(QTime(22, 0));
                }
            });
...
void employe::chargerDepuisUI()
{
    ...
    poste = ui->lineposte_e->currentText();
...
bool employe::ajouter()
{
    if (verifierChamps()) {
        ...
        // Calcul du salaire en fonction du poste
        double salaire = calculerSalaire(poste);

        QSqlQuery query;
        query.prepare("INSERT INTO employe (id_employe, nom, prenom, email, num_tel, poste, salaire, password, email_verifie, code_confirmation) "
                      "VALUES (:id, :nom, :prenom, :email, :num_tel, :poste, :salaire, :password, 0, :code)");
...
double employe::calculerSalaire(const QString &poste) const
{
    QString p = poste.toUpper().trimmed();

    if (p == "DIRECTEUR")
        return 3500.0;   // TND
    else if (p == "MANAGER")
        return 2500.0;
    else if (p == "RESPONSABLE RH")
        return 2200.0;
    else if (p == "OUVRIER")
        return 1800.0;
    else if (p == "STAGIAIRE")
...
{
    // Recalcul du salaire en fonction du poste
    double salaire = calculerSalaire(poste);

    QSqlQuery query;
    query.prepare("UPDATE employe SET nom = :nom, prenom = :prenom, "
                  "email = :email, num_tel = :num_tel, poste = :poste, salaire = :salaire "
                  "WHERE id_employe = :id");

    query.bindValue(":id", id_employe);
    query.bindValue(":nom", nom);
    query.bindValue(":prenom", prenom);
    // ⚠ ici tu bindais :adresse mais il n'existe pas dans le SQL, à retirer ou à ajouter dans le SET
    query.bindValue(":email", email);
    query.bindValue(":num_tel", num_tel);
    query.bindValue(":poste", poste.toUpper());
    query.bindValue(":salaire", salaire);
...
void employe::remplirChampsDepuisTable()
{
    ...
    if (itemPoste)
        this->ui->lineposte_e->setCurrentText(itemPoste->text());
