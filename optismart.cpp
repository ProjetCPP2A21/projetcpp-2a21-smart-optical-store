...
//
    ...
    QTimer *timerRefreshProduits = new QTimer(this);
    connect(timerRefreshProduits, &QTimer::timeout, this, [this]() {

        if (!isFilteringProduits) {  // Only refresh when NOT filtering
            chargerProduits();
        }

    });
    timerRefreshProduits->start(5000); // refresh every 5 seconds
    connect(ui->searchIdEdit, &QLineEdit::textChanged, this, [this](const QString &txt){
        if (txt.trimmed().isEmpty()) {
            isFilteringProduits = false;   // allow auto-refresh again
            chargerProduits();
        }
    });
    connect(ui->recherche_le, &QLineEdit::textChanged, this, [this](const QString &txt){
        if (txt.trimmed().isEmpty()) {
            isFilteringFournisseurs = false;
            actualiserAffichageFournisseur();
        }
    });
    connect(ui->linerechercher_o, &QLineEdit::textChanged, this, [this](const QString &txt){
        if (txt.trimmed().isEmpty()) {
    ...
    actualiserAffichageFournisseur();
    // ---------- AUTO-REFRESH FOURNISSEURS ----------
    auto *timerFournisseurs = new QTimer(this);
    connect(timerFournisseurs, &QTimer::timeout, this, [this]() {
        if (!isFilteringFournisseurs) {
            actualiserAffichageFournisseur();
        }
    });
    timerFournisseurs->start(5000); // 5s

    // ---------- AUTO-REFRESH ORDONNANCES ----------
    auto *timerOrdonnances = new QTimer(this);
    connect(timerOrdonnances, &QTimer::timeout, this, [this]() {
        if (!isFilteringOrdonnances) {
            actualiserAffichageOrdonnance();
        }
    });
    timerOrdonnances->start(5000); // 5s// Fournisseur
    ...
    if (ui->groupBox_6) {
        // Bouton Chatbot - à gauche du bouton Statistique (qui est à x=340)
        QPushButton *btnChatbot = new QPushButton("🤖 Chatbot", ui->groupBox_6);
        btnChatbot->setObjectName("bchatbot_o");
        // x = 340 (stat) - 20 (gap) - 131 (width) = 189
        ...
        // x = 471 + 20 (gap) = 491
        btnPrevision->setGeometry(491, 540, 131, 41);
...
void optismart::on_lineEdit_id_textChanged(const QString &arg1)
{
    bool ok;
...
void optismart::on_searchButton_clicked()
{
    ...
    if (text.isEmpty()) {
        isFilteringProduits = false;  // BACK TO NORMAL MODE
        chargerProduits();
        return;
    }
    isFilteringProduits = true; // STOP AUTO-REFRESH
    bool ok;
...
// ==================== FONCTIONNALITÉS AVANCÉES ORDONNANCE ====================
...
void optismart::on_bexporter_o_clicked()
{
    ord.exporterEnPDF(ui->tableWidget_o);
}

void optismart::on_btrier_o_clicked()
{
    ord.trierParNom(ui->tableWidget_o);
}

void optismart::on_bstatistique_o_clicked()
{
...
void optismart::on_bchatbot_o_clicked()
{
    if (!dialogChatbot) {
        dialogChatbot = new QDialog(this);
        dialogChatbot->setWindowTitle("🤖 Assistant Chatbot");
        dialogChatbot->setMinimumSize(600, 500);
        initialiserChatbot(dialogChatbot);
    }
    dialogChatbot->show();
    dialogChatbot->raise();
    dialogChatbot->activateWindow();
}

void optismart::initialiserChatbot(QWidget *parentWidget)
{
...
void optismart::on_btnChatbotEnvoyer_clicked()
{
    ...
    if (!message.isEmpty()) {
        ...
        QString reponse = chatbot.traiterMessage(message, ui->tableWidget_o);
...
void optismart::on_lineEditChatbot_returnPressed()
{
...
void optismart::on_bprevision_o_clicked()
{
    if (!dialogPrevision) {
        dialogPrevision = new QDialog(this);
        dialogPrevision->setWindowTitle("🔮 Prévision du futur");
        dialogPrevision->setMinimumSize(800, 600);
        initialiserPrevisions(dialogPrevision);
    }
    mettreAJourPrevisions();
    dialogPrevision->show();
    dialogPrevision->raise();
    dialogPrevision->activateWindow();
}
...
void optismart::mettreAJourPrevisions()
{
